...
func TestCollectCoreBuildsPartialSnapshot(t *testing.T) {
	...
	if snapshot.SnapshotID != "ctx-001-20260723T010203Z" {
		t.Fatalf("SnapshotID = %q, want ctx-001-20260723T010203Z", snapshot.SnapshotID)
	}
	if snapshot.CapturedAt != "2026-07-23T01:02:03Z" {
		t.Fatalf("CapturedAt = %q, want 2026-07-23T01:02:03Z", snapshot.CapturedAt)
	}
	if snapshot.Cluster.Context.Name != "ctx-001" {
		t.Fatalf("Context name = %q, want ctx-001", snapshot.Cluster.Context.Name)
	}
	if snapshot.Cluster.Context.KubeconfigSource != "EXPLICIT" {
		t.Fatalf("KubeconfigSource = %q, want EXPLICIT", snapshot.Cluster.Context.KubeconfigSource)
	}
